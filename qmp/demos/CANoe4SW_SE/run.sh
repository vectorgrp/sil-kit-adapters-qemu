...
if [[ -n "$canoe4sw_se_install_dir" ]]; then
  ...
  $script_root/createEnvironment.sh
	...
	#run tests
	$canoe4sw_se_install_dir/canoe4sw-se "$script_root/Default.venvironment" -d "$script_root/working-dir" --verbosity-level "2" --test-unit "$script_root/testPingViaQmp.vtestunit"  --show-progress "tree-element"
	exit_status=$?
else
